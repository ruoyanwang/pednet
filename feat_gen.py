...
for lv in range(num_lv):
    ...
    for src_filename in src_filenames:
        ...
        if lv!=0:
            ...
            feat = conv_out['fc8_inria_conv'][0] 
        else:
            patch0 = img[:, :(320+29), :]
            patch1 = img[:, (320-29):, :]
            conv_in = [convnet.preprocess('data', patch0)]
            conv_out = convnet.forward_all(data=np.asarray(conv_in))
            feat0 = conv_out['fc8_inria_conv'][0]
            conv_in = [convnet.preprocess('data', patch1)]
            conv_out = convnet.forward_all(data=np.asarray(conv_in))
            feat1 = conv_out['fc8_inria_conv'][0]
            feat = np.zeros((2, feat0[1].shape[0], feat0[1].shape[1]+feat1[1].shape[1]))
            feat[:, :, :feat0[1].shape[1]] = feat0
