...
def net_init(src_proto, model, mean_file, device_id=1, dir='inria'):
    "Load a network"
    net = caffe.Net(dir+'/'+src_proto, dir+'/'+model)
    net.set_mode_gpu()
    net.set_device(device_id)
    net.set_phase_test()
    net.set_mean('data', dir+'/'+mean_file)
...
def fullconv_net_init(src_proto, tar_proto, model, mean_file, device_id=1, dir='inria'):
    """ Transfer prototxts and initialize a network"""
    net = caffe.Net(dir+'/'+src_proto, dir+'/'+model)
    """
    patch = caffe.io.load_image('/home/ryan/data/caltech/data-ETH/train/cropped_pos/00010.png')
    out = net.forward_all(data=numpy.asarray(\
                             [net.preprocess('data', patch)]))
    print out.keys()
    print out['pool5'].shape
    """
    params = ['fc6', 'fc7', 'fc8_inria']
    # fc_params = {name: (weights, biases)}
    fc_params = {pr: (net.params[pr][0].data, net.params[pr][1].data) for pr in params}


    net_full_conv = caffe.Net(dir+'/'+tar_proto, dir+'/'+model)
    ...
    net_full_conv.set_mean('data', dir+'/'+mean_file)
